 ...
 * 首字色块 —— 游戏卡的**兜底形态**。
 *
 * ⚠️ 2026-09-17 起种子内容已富化：**47 款游戏全部有 cover**、帖子约 7 成有封面，
 *   所以现在默认形态是「有图显示图」，色块只在两种情况下出现：
 *     ① 接口没给 cover（老数据 / 新录入）；
 *     ② 图片加载失败（`@error` 回退，见 GameTile / PostCard）。
 *   历史上的注释曾写「线上 cover 全是 null」，那个前提**已经不成立**，别再照抄。
 *   颜色按 id 稳定取模，保证同一游戏每次颜色一致。
 */
const TILE_THEMES = [
