...
#
# 依赖：java（调用后端 jar 内置的哈希工具）+ mysql 客户端。
...
[ -f "$JAR" ]      || { echo "[init-admin] 找不到后端 jar：$JAR" >&2
                        echo "             请先构建：cd backend && mvn -o package -DskipTests"
                        echo "             或指定已有路径：YUMU_JAR=/path/to/yumu-community-1.0.0.jar $0" >&2; exit 1; }
command -v "$JAVA_BIN"  >/dev/null 2>&1 || { echo "[init-admin] 找不到 java（可用 JAVA_BIN 指定绝对路径）" >&2; exit 1; }
command -v "$MYSQL_BIN" >/dev/null 2>&1 || { echo "[init-admin] 找不到 mysql 客户端（可用 MYSQL_BIN 指定绝对路径）" >&2; exit 1; }
...
HASH="$("$JAVA_BIN" -jar "$JAR_FOR_JAVA" --gen-password-hash="$PASSWORD" | tail -n 1)"
case "$HASH" in
  \$2a\$*|\$2b\$*|\$2y\$*) : ;;
